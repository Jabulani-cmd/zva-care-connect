...
function MyPrescriptions() {
  ...
  const mine = list.filter((r) => r.customerId === user.id).sort((a, b) => b.submittedAt.localeCompare(a.submittedAt)).slice(0, 4);

  return (
    <div className="bg-white rounded-2xl overflow-hidden">
      <div className="px-5 py-3 flex items-center justify-between gap-2 border-b border-border">
        <div className="flex items-center gap-2">
          <FileText className="h-4 w-4 text-[#1E5BC6]" />
          <div className="font-black text-sm text-[#1B3A6B]">My Prescriptions</div>
        </div>
        <Link to="/prescriptions" className="inline-flex items-center gap-1 text-xs font-bold text-[#1E5BC6] hover:text-[#1B3A6B]">
      ...
      <div className="divide-y divide-border">
        {mine.length === 0 ? (
          <div className="px-5 py-8 text-center">
            <div className="text-sm text-muted-foreground">No prescriptions submitted yet.</div>
            <Link to="/prescriptions" className="inline-block mt-3 bg-[#1B3A6B] hover:bg-[#1E5BC6] text-white text-xs font-bold px-4 py-2 rounded-full transition">Upload prescription</Link>
          </div>
        ) : (
          mine.map((p) => {
            const c = statusColor(p.status);
            return (
              <Link key={p.id} to="/prescriptions" className="px-5 py-3 flex items-center justify-between gap-3 hover:bg-slate-50 transition">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground truncate">{new Date(p.submittedAt).toLocaleDateString()}</div>
                  </div>
                </div>
                <span className="text-[10px] font-bold px-2 py-1 rounded-full shrink-0" style={{ background: c.bg, color: c.fg }}>{p.status}</span>
              </Link>
            );
          })
        )}
